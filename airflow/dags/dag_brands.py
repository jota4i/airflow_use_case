...
def get_brands(vehicle_type, **context):
    ...
    if vehicle_type in vehicle_list:
        ...
        print(
            f"Veiculo do tipo {vehicle_type} não processado por nao estar nos parametros"
        )
...
def transform_land_to_raw(vehicle_type):
    ...
    land_to_raw(
        BUCKET_NAME, f"land/brand/{vehicle_type}.json", f"raw/brand/{vehicle_type}"
    )
...
def transform_raw_to_trusted(vehicle_type):
    ...
    raw_to_trusted(
        BUCKET_NAME,
        f"raw/brand/{vehicle_type}/*",
        f"trusted/brand/{vehicle_type}",
        vehicle_type,
    )
...
with DAG(
    ...
    with TaskGroup(group_id="land") as land_layer:
    ...
    with TaskGroup(group_id="raw") as raw_layer:
        for vtype in VEHICLE_TYPES:
            task_get_brands = PythonOperator(
    ...
    with TaskGroup(group_id="trusted") as trusted_layer:
        for vtype in VEHICLE_TYPES:
            task_get_brands = PythonOperator(
            ...
            task_create_external_table = BigQueryCreateExternalTableOperator(
                ...
                table_resource={
                    ...
                    "externalDataConfiguration": {
                        ...
                        "sourceUris": [
                            f"gs://airflow4i_lake/trusted/brand/{vtype}/*.parquet"
                        ],
                    },
                },
            )
